/** This script generates a vcd file with a sinus signal.
 */

import { writeFileSync } from "fs";

// Parameters:
const cntrPeriod = 100; // in ns
const cntrStart = 0; // init value of the counter
const cntrEnd = 200; // last value of the counter
const cntrStep = 0.1; // step of the counter (use fraction for float values)
const bitWidth = 32; // Width of the counter in bits
const cntrName = "sinsig"; // Name of the clock signal
const vcdId = "`c";
...
function generateCounterVCD() {
  ...
  for (let i = cntrStart; i <= cntrEnd; i += cntrStep) {
    ...
    if (number_format === "float") {
      binValue = floatToBinString(Math.sin(i));
    } else {
      binValue = intToBinString(Math.sin(i), bitWidth);
    }
    vcdLines.push(`b${binValue} ${vcdId}`);
  }

  // Write to file
  writeFileSync(`sinus_${cntrEnd}.vcd`, vcdLines.join("\n"));
}
